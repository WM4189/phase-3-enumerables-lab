...
def print_spicy_foods(spicy_foods)
  ...
  mapped = new_array.map do |food|
    "#{food[0]} (#{food[1]}) | Heat Level: #{food[2]} "
  end
...
def get_spicy_food_by_cuisine(spicy_foods, cuisine)
  certain_food = spicy_foods.find do |food| 
...
def average_heat_level(spicy_foods)
  heat_array = spicy_foods.map do |food|
    food[:heat_level]
    # heat_array
    # [9, 3, 6]
  end
  heat_array.sum {|int| int / 3}
end
